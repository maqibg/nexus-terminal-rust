...
interface RemotePathSuggestionOptions {
    foldersOnly?: boolean;
    electronAPI: any;
}
...
/**
 * 兼容入口：
 * - 旧签名: getRemotePathSuggestions(sessionId, inputPath, electronAPI)
 * - 新签名: getRemotePathSuggestions(sessionId, inputPath, { foldersOnly, electronAPI })
 */
export async function getRemotePathSuggestions(
    sessionId: string,
    inputPath: string,
    optionsOrApi: RemotePathSuggestionOptions | any
): Promise<CompletionItem[]> {
    const options: RemotePathSuggestionOptions = optionsOrApi && optionsOrApi.electronAPI
        ? optionsOrApi
        : { foldersOnly: true, electronAPI: optionsOrApi };

    if (options.foldersOnly) {
        return getRemoteDirectories(sessionId, inputPath, options.electronAPI);
    }
    return getRemoteFiles(sessionId, inputPath, options.electronAPI);
}
