<template>
  <div class="page">
    ...
    <table class="table" v-if="items.length">
      <thead><tr><th>名称</th><th>类型</th><th>主机</th><th>端口</th><th>操作</th></tr></thead>
      <tbody>
        <tr v-for="p in items" :key="p.id">
          <td>{{ p.name }}</td><td>{{ p.proxy_type }}</td><td>{{ p.host }}</td><td>{{ p.port }}</td>
          <td><button class="btn-sm btn-danger" @click="handleDelete(p)">删除</button></td>
        </tr>
      </tbody>
    </table>
    <div v-else class="empty">暂无代理</div>
    <AddProxyForm :visible="showForm" @cancel="showForm = false" @saved="onSaved" />
  </div>
</template>
...
async function handleDelete(p: { id: number; name: string }) {
  if (await confirm('删除代理', `确定删除 "${p.name}" 吗？`)) await store.remove(p.id);
}
