...
impl SettingsRepository for SqliteSettingsRepo {
    ...
    async fn get_theme(&self, id: i64) -> Result<Option<TerminalTheme>, StorageError> {
        ...
        .bind(id).fetch_optional(&self.pool).await.map_err(|e| StorageError(e.to_string()))?;
        Ok(row.as_ref().map(row_to_theme))
    }

    async fn create_theme(&self, t: &TerminalTheme) -> Result<i64, StorageError> {
    ...
    async fn update_theme(&self, t: &TerminalTheme) -> Result<bool, StorageError> {
        let result = sqlx::query(
