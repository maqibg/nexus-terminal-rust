...
impl AuthRepository for SqliteAuthRepo {
    ...
    async fn list_passkeys(&self, user_id: i64) -> Result<Vec<PasskeyRow>, String> {
        sqlx::query_as::<_, (i64, i64, String, String, i64, Option<String>, String)>(
            "SELECT id, user_id, credential_id, public_key, counter, transports, name FROM passkeys WHERE user_id = ?",
        )
        .bind(user_id)
    ...
    async fn find_passkey_by_credential_id(
        ...
        sqlx::query_as::<_, (i64, i64, String, String, i64, Option<String>, String)>(
            "SELECT id, user_id, credential_id, public_key, counter, transports, name FROM passkeys WHERE credential_id = ?",
        )
        .bind(credential_id)
        .fetch_optional(&self.pool).await
    ...
    async fn update_passkey_counter(
        ...
        sqlx::query("UPDATE passkeys SET counter = ? WHERE credential_id = ?")
            .bind(counter as i64)
            .bind(credential_id)
            .execute(&self.pool)
            .await
